...
void BaseGame::render(CameraComponent& camera)
{
	...
	switch (m_cGameState)
	{
		...
		gl::Uniform3f(gl::GetUniformLocation(m_programHandle, "cameraPosition"), camera.position().x, camera.position().y - 20, camera.position().z);
		gl::Uniform3f(gl::GetUniformLocation(m_programHandle, "light.position"), m_robot->getStartPos().x, m_robot->getStartPos().y+1, m_robot->getStartPos().z);
		//Room
